...
void QHomeView::parse(json &_template) {
    ...
    QObject::connect(pool["Main"], SIGNAL(sendStatusMessage(QString)), pool["Footer"], SLOT(setStatus(QString)));
    QObject::connect(pool["loadmodel"], SIGNAL(clicked()), pool["Main"], SLOT(loadModel()));
}
